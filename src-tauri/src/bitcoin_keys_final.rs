...
impl SimpleBitcoinKeyGenerator {
    ...
    pub fn generate_bitcoin_key(
        ...
        // Generate public key and then compressed public key
    ...
    fn generate_address(
        ...
        let address = match key_type {
            BitcoinKeyType::Legacy => {
                Address::p2pkh(&bitcoin_pubkey, network)
            },
            BitcoinKeyType::SegWit => {
                // SegWit P2SH-wrapped address
                Address::p2shwpkh(&bitcoin_pubkey, network)
                    .map_err(|e| anyhow!("Failed to create P2SH-P2WPKH address: {}", e))?
            },
            BitcoinKeyType::Native => {
                // Native SegWit (Bech32) address
                Address::p2wpkh(&bitcoin_pubkey, network)
            ...
            BitcoinKeyType::Taproot => {
                // For now, fallback to Legacy for Taproot
                Address::p2pkh(&bitcoin_pubkey, network)
            },
        };

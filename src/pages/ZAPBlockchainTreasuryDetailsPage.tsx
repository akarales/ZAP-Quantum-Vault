...
import {
  Copy, ArrowLeft, Shield, Key, Network, Clock, Activity, Eye, EyeOff, Trash2, Edit, Download, AlertTriangle, CheckCircle, Wallet, DollarSign
} from 'lucide-react';
...
interface ZAPTreasuryKeyDetails {
  id: string;
  vaultId: string;
  keyType: string;
  keyRole: string;
...
export const ZAPBlockchainTreasuryDetailsPage = () => {
  const { keyId } = useParams<{ keyId: string }>();
  const navigate = useNavigate();
  const [keyDetails, setKeyDetails] = useState<ZAPTreasuryKeyDetails | null>(null);
  ...
  const loadKeyDetails = async () => {
    try {
      ...
      const keys = await invoke<ZAPTreasuryKeyDetails[]>('list_zap_blockchain_keys', {
        vaultId: null,
        keyType: 'treasury'
      });
      ...
      if (!key) {
        setError('Treasury key not found');
        return;
      }
      
      setKeyDetails(key);
    } catch (error) {
      console.error('Failed to load treasury key details:', error);
      setError('Failed to load treasury key details');
    } finally {
      setLoading(false);
    }
  };
  ...
  const decryptPrivateKey = async () => {
    console.log('🔓 Treasury decryptPrivateKey called');
    ...
    console.log('🔑 Treasury Key details:', {
      id: keyDetails.id,
    ...
    console.log('🚀 Starting treasury decryption process');
    ...
    try {
      ...
      console.log('✅ Treasury Decryption successful!');
      ...
      console.log('🎯 Treasury State updated - showPrivateKey: true, decryptedPrivateKey set');
      toast.success('Private key decrypted successfully');
    } catch (error) {
      console.error('❌ Failed to decrypt treasury private key:', error);
      ...
      console.log('🏁 Treasury Decryption process finished, loading state reset');
    }
  };

  const hidePrivateKey = () => {
    console.log('🙈 Treasury hidePrivateKey called');
  ...
  const handleTrashKey = async () => {
    try {
      ...
      toast.success('Treasury key moved to trash');
      navigate('/zap-blockchain/treasury');
    } catch (error) {
      toast.error('Failed to move treasury key to trash');
    }
  };
      ...
      <div className="container mx-auto p-6">
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Loading treasury key details...</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="container mx-auto p-6">
        <div className="flex items-center gap-4 mb-6">
          <Button variant="ghost" size="sm" onClick={() => navigate('/zap-blockchain/treasury')}>
            <ArrowLeft className="h-4 w-4 mr-2" />
            Back to Treasury Keys
          </Button>
        </div>
        
        <Alert variant="destructive">
          <AlertTriangle className="h-4 w-4" />
          <AlertDescription>{error || 'Treasury key not found'}</AlertDescription>
        </Alert>
      </div>
    );
  }
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold">Treasury Key Details</h1>
              <p className="text-muted-foreground flex items-center gap-2">
                <Network className="h-4 w-4" />
                {keyDetails.networkName} • Network Treasury
              </p>
            </div>
          </div>
        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => copyToClipboard(keyDetails.address, 'Treasury Address')}
                    >
                ...
                <div className="text-xs text-gray-500 p-2 bg-gray-50 rounded">
                  Treasury Debug: showPrivateKey={showPrivateKey.toString()}, hasDecryptedKey={!!decryptedPrivateKey}, loading={decryptionLoading.toString()}
                </div>
              </CardContent>
            </Card>
          </div>
        ...
        <TabsContent value="treasury" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Treasury management settings and spending controls
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <Label className="text-sm font-medium">Treasury Status</Label>
                  <p className="mt-1 text-sm">{keyDetails.isActive ? 'Active' : 'Inactive'}</p>
                </div>
                <div>
                  <Label className="text-sm font-medium">Multi-Sig Threshold</Label>
                  <p className="mt-1 text-sm">2 of 3</p>
                </div>
                <div>
                  <Label className="text-sm font-medium">Spending Limit</Label>
                  <p className="mt-1 text-sm">Governance Controlled</p>
                </div>
                <div>
                  <Label className="text-sm font-medium">Budget Period</Label>
                  <p className="mt-1 text-sm">Annual</p>
                </div>
              </div>
              
              <div className="pt-4 border-t">
                <Label className="text-sm font-medium">Treasury Responsibilities</Label>
                <ul className="mt-2 text-sm text-muted-foreground space-y-1">
                  <li>• Manage network funds and reserves</li>
                  <li>• Execute approved budget allocations</li>
          ...
          <Card>
            ...
            <CardContent>
              {keyDetails.metadata && Object.keys(keyDetails.metadata).length > 0 ? (
                ...
                <p className="text-muted-foreground">No metadata available for this treasury key.</p>
              )}
            </CardContent>
          </Card>
        </TabsContent>
